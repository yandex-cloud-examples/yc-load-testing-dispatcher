...
Для использования `диспетчера` нужно запустить инструмент с желаемыми флагами. Например:\
...
После запуска все запросы, которые планируется использовать в нагрузочном тесте можно делать по адресу **localhost:8080**. 

Также можно в браузере открыть страницу **localhost:8080** и должен будет загрузиться сайт, указанный во флаге **target**.

Инструмент можно запускать и без флага **target**. Тогда в качестве целевого сервера будет использован адрес, указанный в заголоввке **Host** запроса. 
